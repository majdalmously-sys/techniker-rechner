...
function saveCurrentDay() {
    ...
    const dayData = {
        date: editingIndex !== null ? history[editingIndex].date : selectedDate,
        entries: entries,
        total: total
    };
    ...
    history.forEach((day, index) => {
        ...
        historyList.innerHTML += `
            <div class="history-item">
                ...
                <br><br>
                <button onclick="showDay(${index})">Anzeigen</button>
            </div>
        `;
    });
...
function showDay(index) {
    ...
    html += `
        <hr>
        <h2 class="day-total">Gesamt: ${day.total.toFixed(2)} €</h2>
        <button class="edit-day" onclick="editDay(${index})">✏️ Bearbeiten</button>
        <button class="delete-day" onclick="deleteDay(${index})">🗑️ Tag lösen</button>
    `;
